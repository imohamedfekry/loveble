...
import { captureErrors } from '../inngest.middleware';
...
export const generateText = inngest.createFunction(
  ...
  async ({ event }) => {
    return captureErrors(
      async () => {
        const modelId =
          (event.data.model as ModelId) ?? 'google:gemini-2.5-flash';
        ...
        for (let i = 0; i < 3; i++) {
          try {
            if (i > 0) {
              console.warn(`[AI] fallback attempt #${i + 1} using ${current}`);
            } else {
            ...
            console.warn(`[AI] provider failed (${current})`, decision);
